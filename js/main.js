...
fetch('./js/data/easter-eggs.json').then(respone => {return respone.json();}).then(jsondata => { eastereggs = jsondata; loaded += 1; if (loaded == 2) { makeReadings(); }});
fetch('./js/data/readings.json').then(respone => {return respone.json();}).then(jsondata => { horoscopes = jsondata; loaded += 1; if (loaded == 2) { makeReadings(); }});
